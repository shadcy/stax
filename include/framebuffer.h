...
i guess we can increase this to 1280x720 16 bit color depth but obviously find
need to make sys optimized for all the nedded stuff. (done)
 * ============================================================================
*/
...
#include <stdint.h>
// making this shit dynamic (not actually dynamic but now we have options)
//  Global framebuffer dimensions (dynamic resolution)
...
void fb_set_resolution(uint32_t w,
                       uint32_t h); // options for 2 kind of resolution
...
void fb_draw_sprite_colorkey(int x, int y, int w, int h, const uint16_t *data,
                             uint16_t colorkey);
